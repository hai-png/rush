 ...
 * has 2FA enabled (mandatory for platform_admin and corporate_admin). Previously this
 * catch block swallowed that error and returned null, so 2FA-enabled users saw a generic
 * "Invalid credentials" message and could not log in via the web app at all.
 *
 * We now surface TwoFactorRequiredError back to the client via a stable error code on
 * the signIn() result (`error: 'TwoFactorRequired'`) so the login page can redirect to
 * a 2FA-code entry step. The actual code is then passed back through `credentials.code`
 * on the second signIn() call, which identityService.login() forwards to otplib's
 * authenticator.check().
 */
      ...
      async authorize(creds, req) {
        ...
        try {
          ...
          if (err instanceof TwoFactorRequiredError) {
            // Throwing here lets NextAuth surface the specific error code to the client
            // via `signIn('credentials', { ... , redirect: false }).error === 'TwoFactorRequired'`.
