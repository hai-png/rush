...
export async function handleTicketMessageWithAttachment(
  req: NextRequest,
  session: any,
  params: any,
  ...
  try {
    ...
    if (ticket.userId !== session.id && session.role !== "platform_admin") {
      return NextResponse.json(
        { error: { code: "FORBIDDEN", message: "Not your ticket", requestId } },
        { status: 403 },
      );
    }
    ...
    if (file && file instanceof File) {
      ...
      try {
        ...
        try {
          await unlink(fileMeta.fullPath);
        } catch {}
        throw err;
      }
    }
    ...
    const notifyUserId =
      session.role === "platform_admin" ? ticket.userId : undefined;
    if (notifyUserId) {
      await enqueueNotification({
        userId: notifyUserId,
        type: "support_reply",
...
export async function PATCH_ticket({
  session,
  params,
  body,
  ipAddress,
