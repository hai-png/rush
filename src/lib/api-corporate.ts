...
export async function POST_onboard({
  ...
  const existing = await db.corporate.findUnique({
  ...
  const corp = await db.$transaction(
    async (tx) => {
      if (session.role !== "platform_admin") {
        ...
        if (!u.phoneVerified)
          throw new ForbiddenError(
            "Phone verification required for this role. Verify your phone first.",
          );
        if (!u.twoFactorEnabled)
          throw new ForbiddenError(
            "Two-factor authentication required for this role. Enable it at /api/v1/auth/2fa/setup.",
          );
      }
  ...
  return {
    ...
    data: {
      corporate: corp,
      message:
        "You are now a corporate admin. Share the invite code with your employees.",
    },
  };
}
  ...
  if (session.role === "platform_admin") {
    if (!corporateId) {
      throw new BadRequestError(
        "platform_admin must supply ?corporateId= query parameter to target a specific corporate",
      );
    }
...
export async function DELETE_invite({
  ...
  if (!invite || invite.corporateId !== corp.id)
    throw new NotFoundError("Invite not found");
  const before = invite;
  await db.corporateInvite.update({
    where: { id: params.id },
    data: { isActive: false },
  });
  await audit({
    actorId: session.id,
    action: "corporate.invite_revoked",
    entityType: "corporate_invite",
    entityId: params.id,
    before,
...
export async function POST_signup({
  ...
  if (invite.usesCount >= invite.maxUses)
    throw new BadRequestError("Invite is full");
  ...
  if (existing) {
    if (existing.approvalStatus === "rejected")
      throw new ConflictError("Your previous request was rejected");
  ...
  return {
    ...
    data: {
      member,
      message: "Request submitted. Your corporate admin will approve you.",
    },
  };
}
...
export async function POST_approve({
  ...
  if (!member || member.corporateId !== corp.id)
    throw new NotFoundError("Member not found");
  if (member.approvalStatus !== "pending")
    throw new ConflictError("Member is not pending");
...
export async function POST_validate_invite({ body }: any) {
  ...
  return {
    data: {
      corporateName: invite.corporate.name,
      subsidyPercent: invite.corporate.subsidyPercent,
...
export async function PATCH_corporate({
  session,
  body,
  query,
  ipAddress,
  ...
  const updated = await db.corporate.update({
    where: { id: corp.id },
    data: input,
  });
  await audit({
...
export async function PATCH_member({
  ...
  const before = member;
  const updated = await db.corporateMember.update({
    where: { id: params.id },
    data: input,
  });
  await audit({
    actorId: session.id,
    action: "corporate.member_updated",
    entityType: "corporate_member",
    entityId: params.id,
    before,
    after: input,
