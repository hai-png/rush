...
export async function POST_pickup({
  session,
  params,
  body,
  ipAddress,
  ...
  if (input.estimatedPickupTime !== undefined)
    data.estimatedPickupTime = input.estimatedPickupTime;
...
export async function DELETE_pickup({
  session,
  params,
  ipAddress,
  userAgent,
...
export async function GET_assignments({ session }: any) {
  ...
  const assignments = await db.routeAssignment.findMany({
    ...
    include: {
      route: {
        include: {
          pickups: { where: { isActive: true }, orderBy: { sortOrder: "asc" } },
        },
      },
      shuttle: true,
      contractor: {
        select: {
          ...
          phone: true,
          contractorProfile: {
            select: { rating: true, verificationStatus: true },
          },
        },
      },
...
export async function POST_assignment({
  session,
  body,
  ipAddress,
  userAgent,
  ...
  if (existing)
    throw new ConflictError(
      "Assignment already exists for this route+contractor+month",
    );
...
export async function generateTripsFromAssignment(
  assignment: any,
): Promise<number> {
  ...
  const holidays = await db.holiday.findMany({
    where: {
      isActive: true,
      date: { gte: assignment.monthStart, lte: assignment.monthEnd },
    },
    select: { date: true },
  });
  ...
  while (cursor <= end) {
    if (
      targetDays.has(cursor.getDay()) &&
      !holidayDates.has(cursor.toDateString())
    ) {
  ...
  logger.info(
    {
      ...
      created,
      total: trips.length,
      holidaysSkipped: holidayDates.size,
    },
    "[assignment] generated trips",
  );
  return created;
}

export async function POST_accept_assignment({
  session,
  params,
  ipAddress,
  userAgent,
  ...
  if (!assignment) throw new NotFoundError("Assignment not found");
  if (
    assignment.contractorId !== session.id &&
    session.role !== "platform_admin"
  ) {
    throw new ForbiddenError("Not your assignment");
  }
  if (assignment.status !== "assigned")
    throw new ConflictError(`Assignment is ${assignment.status}, not assigned`);
  ...
  const freshAssignment = await db.routeAssignment.findUnique({
    where: { id: params.id },
  });
  if (freshAssignment) {
    await generateTripsFromAssignment(freshAssignment).catch((err) => {
      ...
      logger.error(
        { err: (err as Error).message, assignmentId: params.id },
        "[assignment.accept] trip generation failed",
      );
    });
  }
  await db.routeAssignment.update({
...
export async function POST_reject_assignment({
  session,
  params,
  body,
  ipAddress,
  ...
  await audit({
    ...
    entityType: "route_assignment",
    entityId: params.id,
    after: { reason, cancelledTrips: tripsToCancel.length },
    ipAddress,
    userAgent,
  });
  return {
    data: {
      id: params.id,
      status: "cancelled",
      cancelledTrips: tripsToCancel.length,
    },
  };
}
...
export async function GET_my_assignments({ session }: any) {
  ...
  const assignments = await db.routeAssignment.findMany({
    where:
      session.role === "platform_admin" ? {} : { contractorId: session.id },
    include: {
      route: true,
      shuttle: true,
