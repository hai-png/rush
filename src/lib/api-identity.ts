    ...
    if (input.kind === "rider") {
      const user = await db.user.create({
        data: {
          ...
          role: "rider",
          riderProfile: {
            create: { homeArea: input.homeArea, workArea: input.workArea },
          },
        },
      ...
      return {
        ...
        data: {
          ...
          profile: { id: user.riderProfile!.id },
        },
      };
    } else {
      const user = await db.user.create({
      ...
      return {
        ...
        data: {
          ...
          profile: { id: user.contractorProfile!.id },
        },
      };
    }
  } catch (err: any) {
  ...
  if (!ok) {
    ...
    if (shouldLock) {
      await audit({
        ...
        entityType: "user",
        entityId: user.id,
        after: { reason: "max_failed_attempts" },
        ipAddress,
        userAgent,
      });
    }
  ...
  if (user.twoFactorEnabled) {
    ...
    if (input.backupCode) {
      ...
      if (!user.twoFactorSecret)
        throw new ForbiddenError("2FA enabled but no secret. Contact support.");
      const secret = decryptField(user.twoFactorSecret);
      if (!secret)
        throw new ForbiddenError(
          "2FA secret could not be decrypted. Contact support.",
        );
...
export async function POST_refresh({ session, ipAddress, userAgent }: any) {
  ...
  if (sessionRow?.userAgent?.startsWith("impersonated-by:")) {
    throw new ForbiddenError(
      "Cannot refresh an impersonation session. Please sign in directly.",
    );
  }
  ...
  const { token } = await issueSession(user, { userAgent, ipAddress });
  const res = NextResponse.json({
    data: { accessToken: token, expiresIn: 30 * 24 * 3600 },
  });
  await setSessionCookie(res, token);
...
export async function POST_change_password({
  session,
  body,
  ipAddress,
  userAgent,
...
const PhoneChangeRequestInput = z.object({
  newPhone: z
    .string()
    .refine(EthiopianPhone.isValid, "Invalid Ethiopian phone"),
});

export async function POST_phone_change_request({
  session,
  body,
  ipAddress,
  userAgent,
  ...
  await audit({
    actorId: session.id,
    action: "user.phone_change_requested",
    entityType: "user",
    entityId: session.id,
    after: { newPhone },
    ipAddress,
...
export async function POST_phone_change_confirm({
  session,
  body,
  ipAddress,
  userAgent,
  ...
  return {
    data: {
      ok: true,
      message:
        "Phone number updated. Please sign in again with your new phone number.",
    },
  };
}
...
export async function POST_password_reset_confirm({
  body,
  ipAddress,
  userAgent,
}: any) {
  const { phone, code, newPassword } = z
    .object({
      phone: z
        .string()
        .refine(EthiopianPhone.isValid, "Invalid Ethiopian phone"),
  ...
  const user = await db.user.findUnique({
    where: { phone: EthiopianPhone.normalize(phone) },
  });
  if (!user) throw new UnauthorizedError();
  const passwordHash = await hashPassword(newPassword);
  await db.user.update({
...
export async function POST_2fa_enable({ session, body }: any) {
  const { secret, code } = z
    .object({ secret: z.string(), code: z.string().length(6) })
    .parse(body);
  if (!verifySync({ secret, token: code }))
...
export async function POST_2fa_disable({ session, body }: any) {
  const { password, code } = z
    .object({ password: z.string(), code: z.string().length(6).optional() })
    .parse(body);
  const user = await db.user.findUnique({ where: { id: session!.id } });
  if (!user) throw new UnauthorizedError();
  const ok = await verifyPassword(password, user.passwordHash);
