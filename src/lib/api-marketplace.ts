...
export async function POST_create_release({
  session,
  body,
  ipAddress,
  userAgent,
  ...
  if (input.window !== trip.window) {
    throw new BadRequestError(
      `Release window (${input.window}) does not match trip window (${trip.window})`,
    );
  }
  ...
  const release = await db.$transaction(async (tx) => {
    ...
    if (rideCas.count === 0)
      throw new ConflictError(
        "Ride is no longer booked (already released or cancelled)",
      );
    ...
    if (tripCas.count === 0)
      throw new ConflictError(
        "Trip seatsBooked was already 0 — refusing to go negative",
      );
...
export async function POST_claim({
  ...
  if (release.status !== "open")
    throw new ConflictError("Seat release no longer available");
  if (release.expiresAt < new Date())
    throw new BadRequestError("Seat release expired");
  if (release.userId === session.id)
    throw new BadRequestError("Cannot claim your own release");
  ...
  const claim = await db.$transaction(async (tx) => {
    ...
    if (updated.count === 0)
      throw new ConflictError("Seat release was just claimed by someone else");
  ...
  try {
    ...
    await db
      .$transaction(async (tx) => {
        ...
        await tx.seatClaim.updateMany({
          where: { id: claim.id },
          data: { status: "refunded" },
        });
        await tx.seatRelease.updateMany({
          where: { id: release.id, status: "claimed" },
          data: { status: "open" },
        });
      })
      .catch(() => {});
...
export async function GET_my_releases({ session }: any) {
  const releases = await db.seatRelease.findMany({
    ...
    include: {
      trip: { include: { route: true, shuttle: true } },
      claims: {
        include: { claimant: { select: { name: true, phone: true } } },
      },
    },
...
export async function POST_cancel_release({
  session,
  params,
  ipAddress,
  userAgent,
  ...
  if (release.status !== "open")
    throw new BadRequestError(`Cannot cancel a ${release.status} release`);
...
export async function DELETE_release({
  session,
  params,
  ipAddress,
  userAgent,
  ...
  if (release.status !== "open")
    throw new BadRequestError(`Cannot delete a ${release.status} release`);
...
export async function GET_claim({ session, params }: any) {
  ...
  if (
    claim.claimantUserId !== session.id &&
...
export async function POST_claim_direct({
  session,
  body,
  ipAddress,
  userAgent,
