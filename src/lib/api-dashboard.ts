...
export async function GET_rider({ session }: any) {
  const [activeSubs, rides, payments, openTickets, unreadNotifs] = await Promise.all([
  ...
  return { data: { activeSubs, rides, recentPayments: payments, openTickets, unreadNotifs } };
}
...
export async function GET_corporate({ session }: any) {
  const corp = await db.corporate.findUnique({
    ...
    include: {
      members: { include: { user: { select: { name: true, phone: true } } }, orderBy: { createdAt: 'desc' }, take: 50 },
