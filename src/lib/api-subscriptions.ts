...
export function computeCorporateSubsidy(
  priceCents: number,
  subsidyPercent: number,
): { riderAmountCents: number; corporateSubsidyCents: number } {
  const corporateSubsidyCents = Math.round(
    (priceCents * Math.max(0, Math.min(100, subsidyPercent))) / 100,
  );
  return {
...
export async function POST_create({
  ...
  if (plan.isTrial) {
    ...
    if (priorTrial)
      throw new BadRequestError("You have already used the trial plan");
  }
  ...
  if (input.corporateCode) {
    const corp = await db.corporate.findUnique({
      where: { code: input.corporateCode },
    });
    if (!corp || !corp.isActive || corp.deletedAt)
      throw new BadRequestError("Invalid corporate code");
    const member = await db.corporateMember.findUnique({
      where: {
        corporateId_userId: { corporateId: corp.id, userId: session.id },
      },
    });
    if (
      !member ||
      ...
      throw new BadRequestError(
        "You are not an approved member of this corporate",
      );
    }
    corporateId = corp.id;
    const subsidy = computeCorporateSubsidy(
      plan.priceCents,
      corp.subsidyPercent,
    );
  ...
  if (riderAmountCents <= 0) {
    throw new BadRequestError(
      "Corporate subsidy cannot cover 100% of the plan price — please contact support",
    );
  }
  ...
  const sub = await db.$transaction(
    async (tx) => {
      if (plan.isTrial) {
        ...
        if (priorTrialInTx)
          throw new ConflictError("You have already used the trial plan");
      }
  ...
  try {
    ...
    await db.payment
      .updateMany({ where: { reference }, data: { status: "failed" } })
      .catch(() => {});
    throw err;
  }
...
export async function GET_one({ session, params }: any) {
  const sub = await db.subscription.findUnique({
    ...
    include: {
      plan: true,
      payments: true,
      rides: { include: { trip: { include: { route: true } } } },
    },
  });
...
export async function POST_cancel({
  ...
  const before = sub;
  const cancelledRides = await db.$transaction(
    async (tx) => {
      ...
      if (subCas.count === 0)
        throw new ConflictError(
          "Subscription is no longer in a cancellable state",
        );
...
export async function POST_renew({
  session,
  params,
  body,
  ipAddress,
  ...
  if (sub.status === "cancelled")
    throw new ConflictError("Cannot renew a cancelled subscription");
  ...
  const { paymentMethod } = z
    .object({ paymentMethod: z.enum(["telebirr", "cbe"]) })
    .parse(body);
  ...
  if (sub.corporateId) {
    ...
    if (!corp || !corp.isActive || corp.deletedAt) {
      throw new BadRequestError(
        "Your corporate is no longer active — renew without the corporate code or contact support",
      );
    }
    const member = await db.corporateMember.findUnique({
      where: {
        corporateId_userId: { corporateId: corp.id, userId: sub.userId },
      },
    });
    if (
      !member ||
      ...
      throw new BadRequestError(
        "Your corporate membership is no longer active",
      );
    }
    const subsidy = computeCorporateSubsidy(
      sub.plan.priceCents,
  ...
  const newStartDate =
    sub.status === "active" && sub.endDate > now ? sub.endDate : now;
  const newEndDate = new Date(
    newStartDate.getTime() + sub.plan.durationDays * 24 * 3600_000,
  );

  const newSub = await db.$transaction(
    async (tx) => {
