// Admin advanced endpoints — feature parity with original rush.
// Supplements api-admin.ts with: dashboard stats, user management (suspend/
// reactivate/change role), impersonation, pending lists, manual payment
// verification, CSV export, catalog deletes.
...
export async function GET_dashboard() {
  ...
  return {
    data: {
      counts: { users, payments, subs, tickets, auditLogs, contractors, corporates, rides },
      revenueCents,
...
export async function POST_impersonate({ session, params, ipAddress, userAgent }: any) {
  ...
  // Issue a session for the target user, but mark it as impersonated.
  const { token, jti } = await issueSession(target, { userAgent, ipAddress });
  // We need to track that this session is impersonated. The simplest way is
  // to store it in the session row's userAgent field as "impersonated by <admin>".
...
export async function POST_reject_contractor({ session, params, body, ipAddress, userAgent }: any) {
  ...
  await db.contractorProfile.update({
    where: { id: params.id },
    data: { verificationStatus: 'rejected', verificationReason: reason, verifiedById: session.id, verifiedAt: new Date() },
  });
...
export async function GET_pending_corporates() {
  // Corporates don't have a pending state in our schema — they're active by
  // default. Return all corporates for admin overview.
...
export async function GET_admin_subscriptions({ session }: any) {
...
// ─── Manual payment verification (for CBE bank transfers) ───────────────────
...
export async function GET_export_csv({ session, params }: any) {
  ...
  // Fetch all rows — for MVP, no pagination. In production, stream this.
  const model = (db as any)[config.model];
  const rows = await model.findMany({ take: 10000 });

  // Convert to CSV
  if (rows.length === 0) {
    return { data: { csv: '', rowCount: 0 } };
  }
