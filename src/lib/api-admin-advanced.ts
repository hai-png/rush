...
const UserActionInput = z.object({
  ...
  role: z
    .enum(["rider", "contractor", "corporate_admin", "platform_admin"])
    .optional(),
});
...
export async function PATCH_user({
  ...
  if (user.id === session.id)
    throw new BadRequestError("Cannot modify your own account");
  if (input.action === "suspend") {
    ...
    if (!input.role)
      throw new BadRequestError("role is required for change_role");
...
export async function POST_impersonate({
  session,
  params,
  body,
  ipAddress,
  ...
  if (target.role === "platform_admin")
    throw new BadRequestError("Cannot impersonate another admin");
  ...
  const { code } = z
    .object({ code: z.string().length(6).optional() })
    .parse(body ?? {});
  if (!code) {
    throw new BadRequestError(
      '2FA code required for impersonation. Provide your TOTP code in the "code" field.',
    );
  }
  ...
  const { token, jti } = await issueSession(target, {
  ...
  return {
    data: {
      ...
      targetUser: { id: target.id, phone: target.phone, role: target.role },
    },
  };
}
...
export async function POST_reject_contractor({
  session,
  params,
  body,
  ipAddress,
...
export async function POST_activate_corporate({
  session,
  params,
  ipAddress,
  userAgent,
...
export async function DELETE_corporate({
  session,
  params,
  ipAddress,
  userAgent,
  ...
  if (activeSubs > 0) {
    throw new BadRequestError(
      `Cannot deactivate corporate with ${activeSubs} active subscription(s). Expire or cancel them first.`,
    );
  }
  const before = corp;
  await db.corporate.update({
...
export async function GET_admin_subscriptions() {
  const subs = await db.subscription.findMany({
    include: {
      ...
      _count: { select: { payments: true, rides: true } },
    },
    orderBy: { createdAt: "desc" },
    take: 200,
  });
...
export async function POST_verify_payment({
  session,
  params,
  body,
  ipAddress,
  ...
  const { verifiedAmountCents } = z
    .object({ verifiedAmountCents: z.number().int().positive() })
    .parse(body);
  ...
  if (verifiedAmountCents !== payment.amountCents) {
    ...
    await audit({
      ...
      entityType: "payment",
      entityId: params.id,
      after: { expected: payment.amountCents, verified: verifiedAmountCents },
      ipAddress,
      userAgent,
    });
    throw new ConflictError(
      "Verified amount does not match expected amount; payment marked failed",
    );
  }
  ...
  await settlePayment(
    payment.reference,
    Money.fromCents(verifiedAmountCents),
    `manual-verify-${Date.now()}`,
    "Success",
    { manual: true, verifiedBy: session.id },
  );
  await audit({
    actorId: session.id,
...
const EXPORT_TABLES: Record<
  string,
  { model: string; label: string; select?: string[] }
> = {
  users: {
    model: "user",
    label: "Users",
...
export async function GET_export_csv({ session, params }: any) {
  ...
  if (config.select)
    findManyArgs.select = Object.fromEntries(
      config.select.map((k: string) => [k, true]),
    );
...
export async function DELETE_route({
  session,
  params,
  ipAddress,
  userAgent,
  ...
  if (upcomingTrips > 0) {
    throw new BadRequestError(
      `Cannot deactivate route with ${upcomingTrips} upcoming scheduled trip(s). Cancel or complete them first.`,
    );
  }
...
export async function DELETE_shuttle({
  session,
  params,
  ipAddress,
  userAgent,
  ...
  if (upcomingTrips > 0) {
    throw new BadRequestError(
      `Cannot deactivate shuttle with ${upcomingTrips} upcoming scheduled trip(s). Cancel or complete them first.`,
    );
  }
  await db.shuttle.update({
    where: { id: params.id },
    data: { isActive: false },
  });
  await audit({
...
export async function PUT_settings({
  session,
  body,
  ipAddress,
  userAgent,
...
const BulkExpireInput = z.object({
  subscriptionIds: z.array(z.string()).min(1).max(100),
});

export async function POST_bulk_expire({
  session,
  body,
  ipAddress,
  userAgent,
...
const BulkSuspendInput = z.object({
  userIds: z.array(z.string()).min(1).max(100),
});

export async function POST_bulk_suspend({
  session,
  body,
  ipAddress,
  userAgent,
...
export async function PATCH_route_price({
  session,
  params,
  body,
  ipAddress,
...
export async function POST_bulk_refund({
  session,
  body,
  ipAddress,
  userAgent,
  ...
  await audit({
    actorId: session.id,
    action: "admin.bulk_refund",
    entityType: "payment",
    after: { scheduled, skipped, reason: input.reason },
    ipAddress,
    userAgent,
  });
