...
export const subscriptionService = {
  async create(input: CreateSubscriptionInput) {
    ...
    return db.transaction(async (tx) => {
      // Only attach morningSlot/eveningSlot if present — Drizzle's insert values type
      ...
      if (input.morningSlot !== undefined) subValues.morningSlot = input.morningSlot;
      if (input.eveningSlot !== undefined) subValues.eveningSlot = input.eveningSlot;
      const [subRow] = await tx.insert(schema.subscriptions).values(subValues as any).returning();
      const sub = subRow!;
  ...
  async cancel(subscriptionId: string, riderId: string) {
    ...
    return db.transaction(async (tx) => {
      ...
      // If the subscription is active and has a completed payment, queue a prorated
      // refund for the unused rides. The state machine's sideEffects list includes
      // 'refund.if_eligible' — we implement that check here rather than in the
      // state machine itself (which stays pure).
      if (sub.status === 'active') {
        ...
        if (plan && payment && plan.ridesIncluded > 0) {
          ...
          if (unusedRides > 0) {
            ...
            const refundRequestNo = `RF${payment.id}${Date.now()}`;
            await tx.insert(schema.refundRetries).values({
              paymentId: payment.id, merchOrderId: payment.reference, refundRequestNo,
              amount: refundAmount.toString(), reason: 'subscription_cancelled',
            });
          }
        }
      }
