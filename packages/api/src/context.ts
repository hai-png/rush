...
export interface Session {
  ...
  jti: string;
  /**
   * If this session was minted by an admin impersonating another user, this
   * is the admin's userId. The authMiddleware populates it from the JWT's
   * `impersonatedBy` claim so route handlers and audit writers can attribute
   * actions to the admin, not the impersonated user. Null for normal sessions.
   */
